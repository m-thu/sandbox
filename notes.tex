...
\begin{lstlisting}
[label:] (identifier|array_aggregate) := expression;
[label:] (identifier|array_aggregate) <= expression;
...
Attributes that can be used in conjunction with array types are (\lstinline!A! denotes either an array type or object, \lstinline!j=1,2,...! which dimension the attribute shall refer to. If the parentheses are left out, \lstinline!j! defaults to one.):
